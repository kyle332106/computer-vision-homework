...
from .tracking_by_detection import ByteTrackPlateTracker, TrackDetection
...
class PipelineConfig:
    ...
    tracker_kind: str = "bytetrack"  # "bytetrack" | "sort" | "csrt" | "kcf"
    detect_every_n: int = 5
    bytetrack_high_thresh: float = 0.45
    bytetrack_low_thresh: float = 0.10
    bytetrack_match_iou: float = 0.30
    bytetrack_low_match_iou: float = 0.20
...
class ALPRPipeline:
    def __init__(self, cfg: PipelineConfig | None = None):
        ...
        self._tbd_tracker: ByteTrackPlateTracker | None = None
        self._frame_idx = 0
    ...
    def _uses_tracking_by_detection(self) -> bool:
        return self.cfg.tracker_kind.lower() in {"bytetrack", "sort"}

    def _get_tracking_by_detection(self) -> ByteTrackPlateTracker:
        kind = self.cfg.tracker_kind.lower()
        use_low_conf = kind == "bytetrack"
        if self._tbd_tracker is None or self._tbd_tracker.use_low_confidence != use_low_conf:
            self._tbd_tracker = ByteTrackPlateTracker(
                high_thresh=self.cfg.bytetrack_high_thresh,
                low_thresh=self.cfg.bytetrack_low_thresh,
                match_iou=self.cfg.bytetrack_match_iou,
                low_match_iou=self.cfg.bytetrack_low_match_iou,
                max_age=self.cfg.temporal_max_missing_detections,
                min_hits=self.cfg.tracker_min_hits,
                use_low_confidence=use_low_conf,
            )
        else:
            self._tbd_tracker.high_thresh = self.cfg.bytetrack_high_thresh
            self._tbd_tracker.low_thresh = self.cfg.bytetrack_low_thresh
            self._tbd_tracker.match_iou = self.cfg.bytetrack_match_iou
            self._tbd_tracker.low_match_iou = self.cfg.bytetrack_low_match_iou
            self._tbd_tracker.max_age = self.cfg.temporal_max_missing_detections
            self._tbd_tracker.min_hits = self.cfg.tracker_min_hits
        return self._tbd_tracker
    ...
    def _apply_tracking_by_detection(
        self,
        detections: list[Detection],
        frame_rgb: np.ndarray,
    ) -> list[Detection]:
        tracker_inputs = [
            TrackDetection(bbox=det.bbox, score=det.conf, index=i)
            for i, det in enumerate(detections)
        ]
        track_results = self._get_tracking_by_detection().update(tracker_inputs)
        previous_by_id = {tp.track_id: tp for tp in self._tracked}
        next_tracks: list[TrackedPlate] = []
        output: list[Detection] = []

        for result in track_results:
            prev = previous_by_id.get(result.track_id)
            det = detections[result.detection_index] if result.detection_index >= 0 else None
            x1, y1, x2, y2 = result.bbox
            bbox_xywh = self._bbox_xyxy_to_xywh((x1, y1, x2, y2))

            if det is not None:
                text_votes = self._merge_votes(prev, det)
                color_votes = self._merge_color_votes(prev, det)
                det.track_id = result.track_id
                det.text = self._best_vote_text(text_votes, det.text)
                det.normalized_text = self._normalize_raw_plate_text(det.text)
                det.plate_color = self._best_vote_color(color_votes, det.plate_color)
                out_det = det
            else:
                text_votes = self._decay_votes(prev.text_votes) if prev is not None else {}
                color_votes = self._decay_votes(prev.color_votes) if prev is not None else {}
                text = self._best_vote_text(text_votes, prev.text if prev is not None else "")
                color = self._best_vote_color(color_votes, prev.plate_color if prev is not None else "неизвестно")
                h, w = frame_rgb.shape[:2]
            ...
            next_tracks.append(TrackedPlate(
                bbox=bbox_xywh,
                track_id=result.track_id,
                text=out_det.text,
                plate_color=out_det.plate_color,
                tracker=None,
                text_votes=text_votes,
                color_votes=color_votes,
                age=result.age,
                last_detect_age=result.time_since_update,
                missing_frames=result.time_since_update,
            ))
            output.append(out_det)

        self._tracked = next_tracks
        return output
    ...
    def process_stream_frame(self, frame_rgb: np.ndarray) -> list[Detection]:
        if self.cfg.use_tracker and self._uses_tracking_by_detection():
            self._frame_idx += 1
            detections = self.process_frame(frame_rgb)
            return self._apply_tracking_by_detection(detections, frame_rgb)
    ...
    def reset(self) -> None:
        ...
        self._frame_idx = 0
        self._next_track_id = 1
        if self._tbd_tracker is not None:
            self._tbd_tracker.reset()
...
SOURCE_COLORS = {
    ...
    "track": (255, 120, 0),    # синий — Kalman prediction без новой детекции
}
...
def draw_detections(frame_rgb: np.ndarray, detections: list[Detection]) -> np.ndarray:
    ...
    for d in detections:
        ...
        track_prefix = f"#{d.track_id} " if getattr(d, "track_id", 0) else ""
        label = f"{track_prefix}{d.text.upper()}"
